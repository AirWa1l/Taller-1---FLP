...
(define-datatype gate gate?
  (compuerta (gate_id symbol?) (gate_type type?) (input_list input_list?)))
...
(circuito
        (no-empty-gate-list 
        (compuerta 'G1 (not) (no-empty-gate-ref 'A (empty-input-list))) 
        (empty-gate-list)))
...
(circuito
        (no-empty-gate-list 
        (compuerta 'G2 (and) (no-empty-gate-ref 'A (no-empty-gate-ref 'B (empty-input-list))))  
        (empty-gate-list)))
; Ejemplo 3: Combinación de compuertas
; (A OR B) -> G3 -> (NOT G3) -> SALIDA (G4)
(circuito
        (no-empty-gate-list 
        (compuerta 'G3 (or) (no-empty-gate-ref 'A (no-empty-gate-ref 'B (empty-input-list))))  
        (no-empty-gate-list
        (compuerta 'G4 (not) (no-empty-gate-ref 'G3 (empty-input-list)))
        (empty-gate-list))))
...
(circuito
        (no-empty-gate-list 
        (compuerta 'G1 (or) (no-empty-gate-ref 'A (no-empty-gate-ref 'B (empty-input-list))))  
        (no-empty-gate-list
        (compuerta 'G2 (and) (no-empty-gate-ref 'A (no-empty-gate-ref 'B (empty-input-list))))
        (no-empty-gate-list
        (compuerta 'G3 (not) (no-empty-gate-ref 'G2 (empty-input-list)))
        (no-empty-gate-list
        (compuerta 'G4 (and) (no-empty-gate-ref 'G1 (no-empty-gate-ref 'G3 (empty-input-list))))
        (empty-gate-list))))))